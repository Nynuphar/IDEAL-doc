...
If you have an already a list of patients from an ongoing study and you want to import this list to CHIL, please check :ref: `this <BulkRef>`.

Add a patient to an ongoing study
**************************************

Now let's try to add a new patient to an already onboarded study on CHIL (for example, Merengues and Cream). You can do this by searching these patients on the patient list from the hospital database and add them manually. Just navigate to "**Patient management**", and then to "**Add patient to a study**".
...
.. note:: In this search area, you have access to the patients in the Hospital. To search for these patients, CHIL must be connected to the Hospital database and will be regularly updated.
...
Generate a random Study ID using CHIL
========================================

When searching for a patient in the Hospital database (see previous section), you can use CHIL to generate a random Study ID.
